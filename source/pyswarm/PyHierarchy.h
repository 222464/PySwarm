...
struct PyLayerDesc {
    ...
    spatialFilterStride(1),
    recurrentFilterRadius(1),
    numMaps(16),
    actScalar(5.0f),
    ...
    spatialFilterStride(spatialFilterStride),
    recurrentFilterRadius(recurrentFilterRadius),
    numMaps(numMaps),
    actScalar(actScalar),
    recurrentScalar(recurrentScalar),
    poolDiv(poolDiv)
    {}
};
...
public:
    PyHierarchy(
        ...
        const std::vector<PyLayerDesc> &layerDescs, int numArms
    );
